...
export function seed(today = dayKey()) {
  const projects = [
    {
      ...
      title: "六级冲刺",
      category: "study",
      goal: "CET-6 · 向 550 分迈进",
      weekly: 450,
      milestones: ["建立词汇基础", "完成专项训练", "完成模拟测试"],
    },
    {
      id: "product",
      title: "产品设计",
    ...
    },
    {
      ...
      goal: "让运动成为生活的一部分",
      weekly: 240,
      milestones: ["开始规律训练", "建立训练节奏", "保持长期习惯"],
    },
  ];
  const tasks = [
    newTask({
    ...
    newTask({
      ...
      tags: ["个人项目"],
      checklist: [
        { id: uid(), text: "整理用户路径", done: true },
        { id: uid(), text: "设计任务详情", done: false },
        { id: uid(), text: "验证日历交互", done: false },
      ],
    }),
    newTask({
      title: "阅读 · 给自己一点时间",
  ...
  for (let i = 1; i <= 14; i++) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      tasks.push(
        newTask({
          title: ["六级专项练习", "作品集设计", "力量训练"][j],
          category: ["study", "design", "health"][j],
          projectId: projects[j].id,
          date: d,
          start: [600, 840, 1140][j],
          duration: [60, 90, 60][j],
          done,
          actual: done ? [50, 80, 55][j] : 0,
          seriesId: j === 2 ? "seed-fitness" : undefined,
          repeat: j === 2 ? "daily" : "none",
        }),
      );
    }
    focusLogs.push({
